...
class SingleExpenseViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let number = existingExpense?.number{
            amountTextField.text = "\(number)"
        }
    }
    ...
    @IBAction func saveExpense(_ sender: Any) {
        ...
        let number = amountTextField.text
        
        var expense: Expense?
        
        if let existingExpense = existingExpense {
            existingExpense.name = name
            existingExpense.number = number
            
            expense = existingExpense
        }else{
            expense = Expense(name: name, number: number)
        }
